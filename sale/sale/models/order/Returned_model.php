...
class Returned_model extends CI_Model {
	...
	public function get_info($rowid){
		...
		if($query->num_rows() > 0){
			...
			$this->db->select('rh.notify, rh.comment, rh.date_added, rsd.name, rh.return_status_id');
	...
	public function get_list($data){
		...
		$this->db->where('op.store_id', $this->user->getstore_id());
		$this->db->join('order_product AS op', 'r.rowid = op.rowid');
		$return['count']=$this->db->count_all_results($this->db->dbprefix('return') . ' AS r');
	
		$this->db->select('distinct(r.return_id)');
		$this->db->limit($limit, $data['page']);
		
		$this->db->where('op.store_id', $this->user->getstore_id());
		$this->db->join('order_product AS op', 'r.rowid = op.rowid');
		
		$this->db->order_by('r.date_added', 'DESC');
		$this->db->from($this->db->dbprefix('return') . ' AS r');
		$query = $this->db->get();
