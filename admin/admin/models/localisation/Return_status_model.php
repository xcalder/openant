...
class Return_status_model extends CI_Model {
	...
	public function get_return_status($return_status_id='')
	{
		...
		$this->db->where('rs.return_status_id',$return_status_id);
		$this->db->where('rsd.language_id', isset($_SESSION['language_id']) ? $_SESSION['language_id'] : '1');
		$this->db->join('return_status_description AS rsd', 'rsd.return_status_id = rs.return_status_id');
		$this->db->from($this->db->dbprefix('return_status') . ' AS rs');
	...
	//
	public function get_return_statuss_for_setting()
	{
		//
		$this->db->select('rs.return_status_id, rsd.name as status_name');
		
		$this->db->where('rsd.language_id', isset($_SESSION['language_id']) ? $_SESSION['language_id'] : '1');
		$this->db->join('return_status_description AS rsd','rsd.return_status_id = rs.return_status_id');
		
		$this->db->order_by('rs.sort_order', 'ASC');
