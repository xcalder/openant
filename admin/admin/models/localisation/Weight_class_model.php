...
class Weight_class_model extends CI_Model {
	...
	public function get_weight_class($weight_class_id='')
	{
		...
		$this->db->where('wc.weight_class_id',$weight_class_id);
		$this->db->where('wcd.language_id', isset($_SESSION['language_id']) ? $_SESSION['language_id'] : '1');
		$this->db->join('weight_class_description wcd', 'wcd.weight_class_id = wc.weight_class_id');
		$this->db->from($this->db->dbprefix('weight_class') . ' AS wc');
