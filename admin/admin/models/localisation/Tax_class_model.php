...
class Tax_class_model extends CI_Model {
	...
	public function get_tax_class($tax_class_id='')
	{
		...
		$this->db->where('tc.tax_class_id',$tax_class_id);
		$this->db->where('tcd.language_id', isset($_SESSION['language_id']) ? $_SESSION['language_id'] : '1');
		$this->db->join('tax_class_description AS tcd', 'tcd.tax_class_id = tc.tax_class_id');
		
		$this->db->from($this->db->dbprefix('tax_class') . ' AS tc');
	...
	public function get_tax_rule($tax_class_id){
		$this->db->where('tr.tax_class_id', $tax_class_id);
		$this->db->where('tcd.language_id', isset($_SESSION['language_id']) ? $_SESSION['language_id'] : '1');
		$this->db->where('trd.language_id', isset($_SESSION['language_id']) ? $_SESSION['language_id'] : '1');
		
		$this->db->join('tax_rate_description AS trd','trd.tax_rate_id = tr.tax_rate_id');
		$this->db->join('tax_class_description AS tcd','tcd.tax_class_id = tr.tax_class_id');
		$this->db->from($this->db->dbprefix('tax_rule') . ' AS tr');
		$query=$this->db->get();
