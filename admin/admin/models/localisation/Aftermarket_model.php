...
class Aftermarket_model extends CI_Model {
	...
	public function get_aftermarket($aftermarket_id='')
	{
		...
		$this->db->where('a.aftermarket_id',$aftermarket_id);
		$this->db->where('ad.language_id', isset($_SESSION['language_id']) ? $_SESSION['language_id'] : '1');
		$this->db->join('aftermarket_description AS ad', 'ad.aftermarket_id = a.aftermarket_id');
		$this->db->from($this->db->dbprefix('aftermarket') . ' AS a');
