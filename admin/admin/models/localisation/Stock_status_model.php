...
class Stock_status_model extends CI_Model {
	...
	public function get_stock_status($stock_status_id='')
	{
		...
		$this->db->where('ss.stock_status_id',$stock_status_id);
		$this->db->where('ssd.language_id', isset($_SESSION['language_id']) ? $_SESSION['language_id'] : '1');
		$this->db->join('stock_status_description AS ssd', 'ssd.stock_status_id = ss.stock_status_id');
		$this->db->from($this->db->dbprefix('stock_status') . ' AS ss');
