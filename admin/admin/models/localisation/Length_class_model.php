...
class Length_class_model extends CI_Model {
	...
	public function get_length_class($length_class_id='')
	{
		...
		$this->db->where('lc.length_class_id',$length_class_id);
		$this->db->where('lcd.language_id', isset($_SESSION['language_id']) ? $_SESSION['language_id'] : '1');
		$this->db->join('length_class_description AS lcd', 'lcd.length_class_id = lc.length_class_id');
		$this->db->from($this->db->dbprefix('length_class') . ' AS lc');
