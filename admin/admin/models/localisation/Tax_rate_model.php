...
class Tax_rate_model extends CI_Model {
	...
	//
	public function get_tax_rates_for_langugae_id($data=array())
	{
		//
	...
	public function get_tax_rate($tax_rate_id='')
	{
		...
		$this->db->where('tr.tax_rate_id',$tax_rate_id);
		$this->db->where('trd.language_id', isset($_SESSION['language_id']) ? $_SESSION['language_id'] : '1');
		$this->db->where('gzd.language_id', isset($_SESSION['language_id']) ? $_SESSION['language_id'] : '1');
		$this->db->join('tax_rate_description AS trd', 'trd.tax_rate_id = tr.tax_rate_id');
		$this->db->join('geo_zone AS gz', 'gz.geo_zone_id = tr.geo_zone_id');
		$this->db->join('geo_zone_description AS gzd', 'gzd.geo_zone_id = gz.geo_zone_id');
		
		$this->db->from($this->db->dbprefix('tax_rate') . ' AS tr');
