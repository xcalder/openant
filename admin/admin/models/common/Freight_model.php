...
class Freight_model extends CI_Model {
	...
	{
		$this->db->where('f.freight_id', $freight_id);
		$this->db->where('fd.language_id', isset($_SESSION['language_id']) ? $_SESSION['language_id'] : '1');
		$this->db->join('freight_description AS fd','fd.freight_id = f.freight_id');
		$this->db->where('ftd.language_id', isset($_SESSION['language_id']) ? $_SESSION['language_id'] : '1');
		$this->db->join('freight_template_description AS ftd','ftd.freight_template_id = f.freight_template_id');
		$this->db->where('f.store_id', $this->user->getStore_id());
		$this->db->from($this->db->dbprefix('freight') . ' AS f');
	...
	public function get_freight_template($freight_template_id){
		$this->db->where('ft.freight_template_id',$freight_template_id);
		$this->db->where('ft.store_id', $this->user->getStore_id());
		$this->db->where('ftd.language_id', isset($_SESSION['language_id']) ? $_SESSION['language_id'] : '1');
		$this->db->join('freight_template_description AS ftd','ftd.freight_template_id = ft.freight_template_id');
		
		$this->db->from($this->db->dbprefix('freight_template') . ' AS ft');
