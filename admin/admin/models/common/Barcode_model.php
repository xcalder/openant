...
class Barcode_model extends CI_Model {
	...
	{
		$this->db->where('b.barcode_id', $barcode_id);
		$this->db->where('bd.language_id', isset($_SESSION['language_id']) ? $_SESSION['language_id'] : '1');
		$this->db->join('barcode_description AS bd','bd.barcode_id = b.barcode_id');
		$this->db->from($this->db->dbprefix('barcode') . ' AS b');
