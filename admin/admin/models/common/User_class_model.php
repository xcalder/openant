...
class User_class_model extends CI_Model {
	...
	{
		$this->db->select('*');
		$this->db->where('uc.user_class_id',$user_class_id);
		$this->db->where('ucd.language_id', isset($_SESSION['language_id']) ? $_SESSION['language_id'] : '1');
		$this->db->join('user_class_description AS ucd','ucd.user_class_id = uc.user_class_id');
		$this->db->from($this->db->dbprefix('user_class') . ' AS uc');
