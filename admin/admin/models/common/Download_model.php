...
class Download_model extends CI_Model {
	...
	public function get_download($download_id='')
	{
		...
		$this->db->where('d.download_id',$download_id);
		$this->db->where('dd.language_id', isset($_SESSION['language_id']) ? $_SESSION['language_id'] : '1');
		$this->db->join('download_description AS dd', 'dd.download_id = d.download_id');
		$this->db->from($this->db->dbprefix('download') . ' AS d');
