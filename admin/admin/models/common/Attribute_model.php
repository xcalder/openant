...
class Attribute_model extends CI_Model {
	...
	public function get_attribute_for_language($attribute_id)
	{
		...
		$this->db->where('ad.language_id', isset($_SESSION['language_id']) ? $_SESSION['language_id'] : '1');
		$this->db->join('attribute_description AS ad','ad.attribute_id = a.attribute_id');
		$this->db->where('agd.language_id', isset($_SESSION['language_id']) ? $_SESSION['language_id'] : '1');
		$this->db->join('attribute_group_description AS agd','agd.attribute_group_id = a.attribute_group_id');
		$this->db->from($this->db->dbprefix('attribute') . ' AS a');
	...
	public function get_attribute_group($attribute_group_id){
		$this->db->where('ag.attribute_group_id',$attribute_group_id);
		$this->db->where('agd.language_id', isset($_SESSION['language_id']) ? $_SESSION['language_id'] : '1');
		$this->db->join('attribute_group_description AS agd','agd.attribute_group_id = ag.attribute_group_id');
		
		$this->db->from($this->db->dbprefix('attribute_group') . ' AS ag');
