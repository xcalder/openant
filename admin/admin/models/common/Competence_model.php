...
class Competence_model extends CI_Model {
	...
	public function get_competence($competence_id='')
	{
		...
		$this->db->where('c.competence_id',$competence_id);
		$this->db->where('cd.language_id', isset($_SESSION['language_id']) ? $_SESSION['language_id'] : '1');
		$this->db->join('competence_description AS cd', 'cd.competence_id = c.competence_id');
		$this->db->from($this->db->dbprefix('competence') . ' AS c');
