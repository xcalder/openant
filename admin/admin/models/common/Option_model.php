...
class Option_model extends CI_Model {
	...
	public function get_option_for_language($option_id)
	{
		...
		$this->db->where('od.language_id', isset($_SESSION['language_id']) ? $_SESSION['language_id'] : '1');
		$this->db->join('option_description AS od','od.option_id = o.option_id');
		$this->db->where('ogd.language_id', isset($_SESSION['language_id']) ? $_SESSION['language_id'] : '1');
		$this->db->join('option_group_description AS ogd','ogd.option_group_id = o.option_group_id');
		$this->db->from($this->db->dbprefix('option') . ' AS o');
	...
	public function get_option_group($option_group_id){
		$this->db->where('og.option_group_id',$option_group_id);
		$this->db->where('ogd.language_id', isset($_SESSION['language_id']) ? $_SESSION['language_id'] : '1');
		$this->db->join('option_group_description AS ogd','ogd.option_group_id = og.option_group_id');
		
		$this->db->from($this->db->dbprefix('option_group') . ' AS og');
