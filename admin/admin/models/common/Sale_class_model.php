...
class Sale_class_model extends CI_Model {
	...
	{
		$this->db->select('*');
		$this->db->where('sc.sale_class_id',$sale_class_id);
		$this->db->where('scd.language_id', isset($_SESSION['language_id']) ? $_SESSION['language_id'] : '1');
		$this->db->join('sale_class_description AS scd','scd.sale_class_id = sc.sale_class_id');
		$this->db->from($this->db->dbprefix('sale_class') . ' AS sc');
