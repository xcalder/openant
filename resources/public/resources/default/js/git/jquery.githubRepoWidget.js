...
(function() {
	...
	function _renderGitHubWidget(repoEle, repo) {
		...
		if(repo.homepage !== null) {
			_setHtml(_querySelector(repoEle, '.link'), '<a target="_blank" href="'+ repo.homepage +'">'+ repo.homepage +'</a>');
		}
		...
		for (var i = 0; i < github_eles.length; i++) {
			...
			if (! _getAttribute(repoEle, rendered, '')) {
				...
				widget = '<div class="github-box repo">'+
					...
					'<a target="_blank" class="download" href="' + repoUrl + '/zipball/master" title="Get repository">Download as zip</a>'+
					'</div>'+
					'</div>';
