...
<div id="slidshow-<?php echo $row_id;?>" class="carousel slide" data-ride="carousel" data-interval="<?php echo rand(2000, 4500);?>">
	...
	<div class="carousel-inner">
		...
		<div class="item <?php echo ($k == 0) ? 'active' : '';?>">
			<a <?php echo !empty($banners[$k]['link']) ? 'target="_blank" target="_blank" href="'.$banners[$k]['link'].'"' : '';?>><img width="<?php echo $setting['width'];?>px" height="<?php echo $setting['height'];?>px" class="lazy lazy-auto" data-original="<?php echo $banners[$k]['image'];?>" title="<?php echo $banners[$k]['title']?>" alt="<?php echo $banners[$k]['title']?>"></a>
		</div>
		<?php endforeach;?>
	</div>
</div> 
