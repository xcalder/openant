...
class Image_common extends CI_Common {
	...
	public function resize($filename, $width, $height, $master_dim='h') {
		if(strstr($filename, 'http://') || strstr($filename, 'https://')){
		    if(!is_file(IMGPATH . '/cache/web_img/' . md5($filename))){
		        $this->getImage($filename, IMGPATH.'/cache/web_img', md5($filename), 1);
		    }
		    return 'resources/image/cache/web_img/' . md5($filename);
			//return $filename;
			exit();
		}
		...
		if (!is_file(IMGPATH . $filename)) {
			...
			return 'resources/image/cache/no_image/'.$no_image;
		}
		...
		return 'resources/image' . $new_image;
	}
